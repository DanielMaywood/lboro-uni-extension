const createTimetableExport = () => {
  ...
  button.addEventListener("click", () => {
    ...
    function msToVeventDate(ms) {
      ...
      return `${date.getFullYear()}${padZeroes(
        date.getUTCMonth() + 1
      )}${padZeroes(date.getUTCDate())}T${padZeroes(
        date.getUTCHours()
      )}${padZeroes(date.getUTCMinutes())}${padZeroes(date.getUTCSeconds())}Z`;
    }
    ...
    (function main() {
      ...
      const weekStartDates = [
        ...
        ...$("#P2_MY_PERIOD > option")
          ...
          .map((x) =>
            /^Sem \d - Wk \d{1,2} \(starting (\d{1,2}-[A-Z]{3}-\d{4})\)$/.exec(
              x
            )
          )
